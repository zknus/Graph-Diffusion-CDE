...
class ODEFuncLapCONV(ODEFunc):
  ...
  def __init__(self, in_features, out_features, opt, data, device):
    ...
    self.weight_mlp = nn.Parameter(torch.FloatTensor(in_features, out_features).to(device))
    self.output_low, self.output_high (
  ...
  def forward(self, t, x):  # t is needed when called by the integrator
    ...
    # ax3 is the divergence of the V elementwise product with X in the paper
    ...
    # ax2 is the diffusion item in the paper
